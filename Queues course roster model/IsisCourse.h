...
private:
	...
	// tries to remove student from the waitlist
	// returns true if removal is successful (false if student not found)
	bool remove_student_from_waitlist(Queue *waitlist, Student s, 
					bool found_student);
	
	// returns waitlist position of student
	int find_position(Queue *waitlist, Student s);
	
	// returns whether or not student is on waitlist 
	bool is_on_waitlist(Queue *waitlist, Student s);
};
